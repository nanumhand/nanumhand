...
<%
	...
	String Qtxt1="select * from(	select rownum as R, C.* from( select A.user_id, B.* from tbl_user A, tbl_board B where A.seq = B.seq_user start with	 B.parent=0 connect by prior B.seq = B.parent order siblings by B.create_date desc) C where rownum <= 30) where R >= 1";
	...
	while(rset.next()) {
		...
		jsonObj.put("parent",rset.getInt(4));
...
d3.json("cookbook.json", function(error, ss) {
