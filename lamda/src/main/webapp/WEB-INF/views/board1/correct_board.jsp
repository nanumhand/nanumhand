...
<%
	...
	String Qtxt1="select title, category, content from tbl_board where seq ="+seq_board;	
	...
	rset =stmt.executeQuery("select board_cat from tbl_category");
%>
<div id="absolute2"><img src="/img/logo_small.png" style="height:75px; width:120px"></div>
 <div id="absolute1">
 <form action="updateDB.jsp" method="post" id="frm" onSubmit="return submitContents(this)">
 <table border="0" cellpadding="1" cellspacing="1" style="width:682px">
	...
	int i=0;
	while(rset.next()){
		String append=(rset.getString(1).equals(category))? "selected": "";
	...
	%>
			...
			<input maxlength="80" id="title" name="title" size="70" type="text"  value="<%=title%>" onFocus="if(this.value=='제목을 입력하세요.'){this.value=''; this.className='myColor2';}" 
	onBlur="if(this.value==''){this.value='제목을 입력하세요.'; this.className='myColor1';}" 
	class="myColor1" /></td>
		</tr>
		<tr>
			<td colspan=2><textarea name="textAreaContent" id="textAreaContent" rows="10" cols="10" style="width:675px; height:500px;"><%=content%>
    </textarea></td>
		</tr>
		<tr>
